-- =====================================================================
-- 08_role_based_access_fix_v2.sql
...
-- FIXED VERSION: Correct column names based on actual table structure
-- =====================================================================
...
-- (TM_INTERACCION uses DESCRIPCION, not NOTAS)
-- =====================================================================
...
-- (TM_NOTIFICACION uses LEIDA not LEIDO, no IDEMPRESA column)
-- =====================================================================
...
BEGIN
    ...
    -- Notificaciones (using correct column names: LEIDA, FECHACREACION)
    SELECT 
        n.IDNOTIFICACION,
        n.TITULO,
        n.MENSAJE,
    ...
    -- Unread count (using LEIDA not LEIDO)
...
-- =====================================================================
-- Add more notifications (using correct column names)
-- =====================================================================
PRINT '';
...
-- Add notifications for different users (using correct columns)
INSERT INTO TM_NOTIFICACION (IDUSUARIO, TITULO, MENSAJE, TIPO, LEIDA, ACTIVO)
VALUES
    -- Admin notifications
    (1, 'Reporte semanal disponible - role-based', 'El reporte de ventas de la semana está listo', 'INFO', 0, 1),
    (1, 'Nuevo ejecutivo registrado - role-based', 'Se ha registrado un nuevo ejecutivo en el sistema', 'SISTEMA', 0, 1),
    
    -- Supervisor1 notifications
    (2, 'Meta del mes alcanzada - role-based', 'Tu equipo ha alcanzado el 100% de la meta mensual', 'EXITO', 0, 1),
    (2, 'Seguimiento pendiente urgente - role-based', 'Hay 3 seguimientos urgentes sin atender', 'ALERTA', 0, 1),
    
    -- Supervisor2 notifications
    (3, 'Nuevo cliente asignado - role-based', 'Se ha asignado un nuevo cliente a tu equipo', 'INFO', 0, 1),
    
    -- Ejecutivo1 notifications
    (4, 'Cita confirmada - role-based', 'Tu cita con Empresa ABC ha sido confirmada', 'INFO', 0, 1),
    (4, 'Seguimiento vencido - role-based', 'Tienes un seguimiento que venció hace 2 días', 'ALERTA', 0, 1),
    (4, 'Nuevo contacto registrado - role-based', 'Se ha registrado un nuevo contacto para Empresa XYZ', 'INFO', 0, 1),
    
    -- Ejecutivo2 notifications
    (5, 'Recordatorio de reunión - role-based', 'Tienes una reunión programada para mañana', 'RECORDATORIO', 0, 1),
    
    -- Ejecutivo3 notifications
    (6, 'Cliente actualizado - role-based', 'La información del cliente ha sido actualizada', 'INFO', 0, 1);

PRINT '  Added notifications for all users';
GO

-- =====================================================================
-- Add SEGUIMIENTO_DETALLE for empresas (using correct column names)
-- Columns: TIPOCOMUNICACION, FECHA1ERCONTACTO, ESTATUSCLIENTE, 
--          DETALLEESTATUS, TIPOLLAMADA, PRESUPUESTO, OBSERVACIONES
-- =====================================================================
PRINT '';
PRINT 'Adding seguimiento details for empresas...';
GO

-- Add details for seguimientos using correct column names
DELETE FROM TM_SEGUIMIENTO_DETALLE WHERE OBSERVACIONES LIKE '%role-based%';

INSERT INTO TM_SEGUIMIENTO_DETALLE (IDSEGUIMIENTO, TIPOCOMUNICACION, FECHA1ERCONTACTO, ESTATUSCLIENTE, DETALLEESTATUS, TIPOLLAMADA, PRESUPUESTO, OBSERVACIONES, USUARIOCREA, ACTIVO)
SELECT TOP 20
    s.IDSEGUIMIENTO,
    CASE (s.IDSEGUIMIENTO % 4)
        ...
        WHEN 2 THEN 'VISITA'
        ELSE 'REUNION'
    END AS TIPOCOMUNICACION,
    DATEADD(DAY, -30, s.FECHAPROGRAMADA) AS FECHA1ERCONTACTO,
    CASE (s.IDSEGUIMIENTO % 3)
        WHEN 0 THEN 'INTERESADO'
        WHEN 1 THEN 'EN_EVALUACION'
        ELSE 'PROSPECTO'
    END AS ESTATUSCLIENTE,
    'Detalle del estado del cliente: ' + e.NOMBRECOMERCIAL AS DETALLEESTATUS,
    CASE (s.IDSEGUIMIENTO % 2)
        WHEN 0 THEN 'ENTRANTE'
        ELSE 'SALIENTE'
    END AS TIPOLLAMADA,
    (s.IDSEGUIMIENTO * 1000) + 5000 AS PRESUPUESTO,
    'Observaciones del seguimiento: Cliente contactado satisfactoriamente. Se acordó próximo paso. role-based' AS OBSERVACIONES,
    s.IDUSUARIOASIGNADO,
    1
FROM TM_SEGUIMIENTO s
INNER JOIN TM_EMPRESA e ON s.IDEMPRESA = e.IDEMPRESA
...
SELECT u.IDUSUARIO, u.LOGINUSUARIO AS USUARIO, p.DESCRIPCION AS PERFIL, u.IDSUPERVISOR,
       s.LOGINUSUARIO AS SUPERVISOR_NAME
FROM TM_USUARIO u
...
INNER JOIN TM_USUARIO u ON s.IDUSUARIOASIGNADO = u.IDUSUARIO
WHERE s.ACTIVO = 1
GROUP BY u.LOGINUSUARIO, s.ESTADO
ORDER BY u.LOGINUSUARIO, s.ESTADO;
...
SELECT u.LOGINUSUARIO AS USUARIO, COUNT(*) AS CompletadosEstaSemana
FROM TM_SEGUIMIENTO s
INNER JOIN TM_USUARIO u ON s.IDUSUARIOASIGNADO = u.IDUSUARIO
WHERE s.ESTADO = 'COMPLETADO'
...
SELECT u.LOGINUSUARIO AS USUARIO, COUNT(*) AS TotalNotificaciones, SUM(CASE WHEN n.LEIDA = 0 THEN 1 ELSE 0 END) AS NoLeidas
FROM TM_NOTIFICACION n
