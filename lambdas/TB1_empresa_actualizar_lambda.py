"""
Lambda Function: TB1_empresa_actualizar_lambda
Description: Update company data for BÚSQUEDA view
API Endpoint: PUT /empresa
Runtime: Python 3.13
...
def get_cors_headers(methods='PUT,OPTIONS'):
    """Return CORS headers."""
    return {
...
def lambda_handler(event, context):
    ...
    try:
        ...
        # Extract and validate idEmpresa
        idEmpresa = body.get('idEmpresa')
        if not idEmpresa:
            return error_response('idEmpresa es requerido', 400)
        
        try:
            idEmpresa = int(idEmpresa)
            if idEmpresa <= 0:
                return error_response('idEmpresa inválido', 400)
        except ValueError:
            return error_response('idEmpresa debe ser numérico', 400)
        
        # Extract optional parameters (sanitize strings)
        # Accept both camelCase and UPPERCASE field names
        ...
        def get_field(body, camel_key, upper_key):
            """Get field value from either camelCase or UPPERCASE key"""
        ...
        usuarioModifica = get_field(body, 'usuarioModifica', 'USUARIOMODIFICA') or 1
        ...
        # Execute stored procedure - includes @ContactoCargo parameter
        params = (
            idEmpresa, nombreComercial, razonSocial, ruc, sedePrincipal, domicilio,
            ...
            actividadEconomica, riesgo, numTrabajadores, int(usuarioModifica)
        )
        
        cursor.execute('''
            EXEC usp_ActualizarEmpresa 
                @IdEmpresa=?, @NombreComercial=?, @RazonSocial=?, @RUC=?, @SedePrincipal=?, @Domicilio=?,
                @ContactoNombre=?, @ContactoEmail=?, @ContactoTelefono=?, @ContactoCargo=?,
                @TipoCliente=?, @LineaNegocio=?, @SublineaNegocio=?, @TipoCredito=?, @TipoCartera=?,
                @ActividadEconomica=?, @Riesgo=?, @NumTrabajadores=?, @UsuarioModifica=?
        ''', params)
        ...
        logger.info(f"Updated empresa {idEmpresa}, rows affected: {rows_affected}")
        return success_response({'rowsAffected': rows_affected}, 'Empresa actualizada correctamente')
        
    except pyodbc.Error as e:
        logger.error(f"Database error in empresa_actualizar: {str(e)}")
        ...
        logger.error(f"Unexpected error in empresa_actualizar: {str(e)}")
