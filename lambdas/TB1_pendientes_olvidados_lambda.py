"""
Lambda Function: TB1_pendientes_olvidados_lambda
Description: Get overdue pending items for PENDIENTES OLVIDADOS view
API Endpoint: GET /pendientes-olvidados?userId=1&prioridad=Alta&fechaHasta=2025-12-10
Runtime: Python 3.13
...
def lambda_handler(event, context):
    """
    Get overdue pending items (past follow-ups not completed).
    
    Query Parameters:
        - userId: User ID (required)
        - prioridad: Filter by priority (optional): Alta, Media, Baja
        - fechaHasta: Filter up to date YYYY-MM-DD (optional)
    
    Response:
        {
            "isSuccess": true,
            "data": [ list of overdue items with DIAS_ATRASADOS ]
        }
    """
    ...
    try:
        ...
        fecha_hasta = query_params.get('fechaHasta')
        ...
        if fecha_hasta:
            fecha_hasta = str(fecha_hasta).strip()[:10]
        ...
        cursor.execute(
            'EXEC usp_ObtenerPendientesOlvidados @IdUsuario=?, @DiasAntiguedad=?, @Prioridad=?, @FechaHasta=?',
            (user_id, 7, prioridad, fecha_hasta)
        )
        ...
        logger.info(f"Pendientes olvidados for user {user_id}: {len(data)} items (filters: prioridad={prioridad}, fechaHasta={fecha_hasta})")
        return success_response(data)
        
    except pyodbc.Error as e:
        logger.error(f"Database error in pendientes_olvidados: {str(e)}")
        return error_response('Error de base de datos', 500)
        
    except Exception as e:
        logger.error(f"Unexpected error in pendientes_olvidados: {str(e)}")
